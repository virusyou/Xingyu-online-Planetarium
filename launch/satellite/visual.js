...
//     //读取完成后，数据保存在对象的result属性中
    // console.log("result   :",this.result.type);
    // var csvdata=await FileAttachment("example.csv").text()
    d3.csv("example.csv").then(function(csvdata){
        var data = csvdata;
        try {
          draw(data);
        } catch (error) {
