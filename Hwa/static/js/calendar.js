const calendar = {
    ...
    solar2lunar: function(yPara, mPara, dPara) {
        ...
        return {
            "阳历日期": solarDate,
            "农历日期": lunarDate,
            '生肖': this.getAnimal(year),
            '农历月': (isLeap ? "\u95f0" : '') + this.toChinaMonth(month),
            '农历日': this.toChinaDay(day),
            '干支年': gzY,
            '干支月': gzM,
            '干支日': gzD,
            '星期几': "\u661f\u671f" + cWeek,
    ...
    ciyundata: function(yPara, mPara, dPara) {
        let y = parseInt(yPara);
        let m = parseInt(mPara);
        let d = parseInt(dPara);
        //年份限定、上限
        ...
        //天干地支处理
        const sm = m - 1;
        const gzY = this.toGanZhiYear(year) + '年';
        ...
        // 依据12节气修正干支月
        let gzM = this.toGanZhi((y - 1900) * 12 + m + 11) + "月";
        if (d >= firstNode) {
            gzM = this.toGanZhi((y - 1900) * 12 + m + 12) + "月";
        }
        ...
        //日柱 当月一日与 1900/1/1 相差天数
        const dayCyclical = Date.UTC(y, sm, 1, 0, 0, 0, 0) / 86400000 + 25567 + 10;
        const gzD = this.toGanZhi(dayCyclical + d - 1) + "日";
        //该日期所属的星座
        const astro = this.toAstro(m, d);

        const solarDate = y + '-' + m + '-' + d;
        const lunarDate = year + '-' + month + '-' + day;
        const ani = this.getAnimal(year);
        const IMonthCn = (isLeap ? "\u95f0" : '') + this.toChinaMonth(month);
        const IDayCn = this.toChinaDay(day);
        const nongli = IMonthCn + IDayCn;
        const ncWeek = "星期" + cWeek;
        return { solarDate, lunarDate, ani, nongli, gzY, gzM, gzD, ncWeek, Term, astro }
    },
